        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2')
        {
            ...
            if (s != "") 
            {
                ...
                if (event == "telemetry") 
                {
                    ...
                    double distance_difference = 15.0;
                    double heading_difference = 0.0;
                    double heading_to_target = 0.0;
                    
                    // if we're far from target predict far future (dt=0.3 sec)
                    if (distance_difference > 0.5)
                    {
                        double dt = 0.3;
                        VectorXd state = ukf.PredictionCatch(dt);
                        target_x = state[0];
                        target_y = state[1];
                    ...
                    // if we're closing to target in order not to overshoot let's predict near future (dt=0.1 sec)
                    if (distance_difference <= 0.5)
                    {
                        double dt = 0.1;
                        VectorXd state = ukf.PredictionCatch(dt);
